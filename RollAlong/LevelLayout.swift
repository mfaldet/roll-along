...
struct LevelLayout {
    ...
    /// Challenge of the Day — a deliberately BRUTAL, coin-free gauntlet, far
    /// harder than any climb level.  A serpentine of hole-walls with narrow,
    /// staggered gaps the ball must thread in sequence; one touch and you fall.
    /// `seed` (the calendar day + sub-level) makes it deterministic, so everyone
    /// faces the same one.  Not time-scored — pure survival to the goal.
    ///
    /// Difficulty here is real (precision threading under tilt momentum), NOT a
    /// high level number — climb levels stay within one easy-ish window; this
    /// is intentionally outside it, with a high per-attempt failure rate.
    static func dailyChallenge(seed: Int) -> LevelLayout {
        var h = UInt64(bitPattern: Int64(seed)) &* 0x9E3779B97F4A7C15
        func rnd() -> CGFloat {
            h = h &* 6364136223846793005 &+ 1442695040888963407
            return CGFloat(h >> 40) / CGFloat(1 << 24)   // 0..<1
        }

        var holes: [CGRect] = [
            CGRect(x: 0.00, y: 0, width: 0.12, height: 1),   // side walls
            CGRect(x: 0.88, y: 0, width: 0.12, height: 1),
        ]
        let rows = 5 + Int(rnd() * 3)                 // 5…7 lethal rows
        let bandH: CGFloat = 0.06                       // tall enough to catch a fast ball
        let gapW:  CGFloat = 0.15                       // narrow — precision required
        let top: CGFloat = 0.14, bottom: CGFloat = 0.86
        var lastGap: CGFloat = 0.5
        for i in 0..<rows {
            let y = top + (bottom - top) * CGFloat(i) / CGFloat(rows - 1) - bandH / 2
            // Force a big lateral move from the previous gap — that's the brutality.
            var gx = 0.20 + rnd() * 0.60
            if abs(gx - lastGap) < 0.28 { gx = lastGap < 0.5 ? gx + 0.30 : gx - 0.30 }
            gx = min(0.80, max(0.20, gx))
            lastGap = gx
            let leftEnd    = gx - gapW / 2
            let rightStart = gx + gapW / 2
            if leftEnd    > 0.12 { holes.append(CGRect(x: 0.12, y: y, width: leftEnd - 0.12, height: bandH)) }
            if rightStart < 0.88 { holes.append(CGRect(x: rightStart, y: y, width: 0.88 - rightStart, height: bandH)) }
        }
        return LevelLayout(
            holeRects:  holes,
            start:      UnitPoint(x: 0.5, y: 0.94),
            goal:       UnitPoint(x: 0.5, y: 0.06),
            coins:      [],                    // no coins on daily maps
            targetTime: 999, goldTime: 999,    // not time-scored
            tier:       .veryHard,
            verified:   true                   // generated by hand-tuned code; don't auto-modify
        )
    }
...
extension LevelLayout {
    ...
    /// Optional compiled-in fallback layer for climb levels, sitting between
    /// the JSON overrides and the procedural generator in `layout(for:)`
    /// (override → handCrafted → generated).
    ///
    /// **Intentionally empty.** All 100 shipped climb levels are the single
    /// source of truth in the bundled `LevelOverrides.json`, authored in Marble
    /// Mapper and migrated out of this array by
    /// `marble-mapper/scripts/migrate-climb-to-overrides.mjs`. Storing them as
    /// JSON instead of a ~2,000-line Swift array literal removes a multi-second
    /// type-check and makes levels overwritable by dropping in a freshly
    /// exported `LevelOverrides.json` — no Swift change, no app-logic change.
    ///
    /// Add entries here with `make(...)` only for a level that must ship even if
    /// the JSON fails to load; otherwise author in Marble Mapper.
